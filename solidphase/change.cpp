...
// File ext. is revised as "tec" from "plt".
*/
...
//	wetrho = gammas/kappa1;
	...
	for(int i=0; i<(XCELL_NUM+6); i++) {
		for(int j=0; j<(YCELL_NUM+6); j++) {
			...
			cellp->momentum_x = RHOmin*Uinf*(1.0-((double)j - 28.0)/25.0*((double)j - 28.0)/25.0);
			cellp->momentum_y = 0.0;
			cellp->v_x = Uinf*(1.0-((double)j - 28.0)/25.0*((double)j - 28.0)/25.0);
			cellp->v_y = 0.0;
			cellp->rho = RHOmin;
			cellp->rhonew = RHOmin;
			cellp->temper = Ts;
	...
	for(int i=2; i<(XCELL_NUM+5); i++) {
		for(int j=2; j<(YCELL_NUM+5); j++) {
			...
			if((i==25)&&(j==25)) energystart=cellp->energy;
		}
	}
	...
	for(int i=3; i<=(XCELL_NUM+3); i++){
		for(int j=3; j<=(YCELL_NUM+3); j++){
			...
			if((j!=3)&&(j!=53)){
				...
				cellp->rhonew = cellp->rho + 0.5*dyi*dt*(4.0*cellp_ym->momentum_y-cellp_ymm->momentum_y);
			}
	...
	for(int i=3; i<=(XCELL_NUM+3); i++){
		for(int j=3; j<=(YCELL_NUM+3); j++){
			...
			if((j!=3)&&(j!=53)){
				...
				cellp->rho = 0.5*(cellp->rhonew+cellp->rho + 0.5*dyi*dt*(4.0*cellp_ym->momentum_ynew-cellp_ymm->momentum_ynew));
			}
	...
	if(flagnew == 0){
		for(int i=2; i<(XCELL_NUM+5); i++) {
			for(int j=2; j<(YCELL_NUM+5); j++) {
				...
				cellp->mu = MUG+(MUL-MUG)/(RHOmax-RHOmin)*(cellp->rhonew-RHOmin);
				
				if(cellp->mu > RHOmax) cellp->mu = MUL;
				else if(cellp->mu < RHOmin){
					cellp->mu = MUG;
				}
			}
		}
		...
		for(int i=2; i<(XCELL_NUM+5); i++) {
			for(int j=2; j<(YCELL_NUM+5); j++) {
				...
				cellp->mu = MUG+(MUL-MUG)/(RHOmax-RHOmin)*(cellp->rho-RHOmin);
...
void momentumnewbnd(void){
	...
	for (int j=0; j<(YCELL_NUM+6); j++){
		...
		cellp_x0->momentum_xnew   = RHOmin*Uinf*(1.0-((double)j - 28.0)/25.0*((double)j - 28.0)/25.0);
		cellp_x1->momentum_xnew   = RHOmin*Uinf*(1.0-((double)j - 28.0)/25.0*((double)j - 28.0)/25.0);
		cellp_x2->momentum_xnew   = RHOmin*Uinf*(1.0-((double)j - 28.0)/25.0*((double)j - 28.0)/25.0);
		cellp_xcp3->momentum_xnew = cellp_xcp2->momentum_xnew;
		cellp_xcp4->momentum_xnew = cellp_xcp2->momentum_xnew;
		cellp_xcp5->momentum_xnew = cellp_xcp2->momentum_xnew;
		cellp_x0->momentum_ynew   = 0.0;
		cellp_x1->momentum_ynew   = 0.0;
		cellp_x2->momentum_ynew   = 0.0;
		cellp_xcp3->momentum_ynew = cellp_xcp2->momentum_ynew;
		cellp_xcp4->momentum_ynew = cellp_xcp2->momentum_ynew;
		cellp_xcp5->momentum_ynew = cellp_xcp2->momentum_ynew;
	}
#endif
/*
#ifndef WALLX
...
*/
	...
	for(int i=0; i<(XCELL_NUM+6); i++){										//ï«ñ 
		...
		cellp_ycp3->momentum_xnew = 0.0;
		cellp_ycp4->momentum_xnew = cellp_ycp2->momentum_xnew;
		cellp_ycp5->momentum_xnew = cellp_ycp1->momentum_xnew;
		...
		cellp_ycp3->momentum_ynew = 0.0;
		cellp_ycp4->momentum_ynew = -cellp_ycp2->momentum_ynew;
		cellp_ycp5->momentum_ynew = -cellp_ycp1->momentum_ynew;
	}
#endif
/*
#ifdef WALLY
...
void vnewbnd(void){
	...
	for (int j=0; j<(YCELL_NUM+6); j++){
		...
		cellp_x0->v_xnew   = Uinf*(1.0-((double)j - 28.0)/25.0*((double)j - 28.0)/25.0);
		cellp_x1->v_xnew   = Uinf*(1.0-((double)j - 28.0)/25.0*((double)j - 28.0)/25.0);
		cellp_x2->v_xnew   = Uinf*(1.0-((double)j - 28.0)/25.0*((double)j - 28.0)/25.0);
		cellp_xcp3->v_xnew = cellp_xcp2->v_xnew;
		cellp_xcp4->v_xnew = cellp_xcp2->v_xnew;
		cellp_xcp5->v_xnew = cellp_xcp2->v_xnew;
		cellp_x0->v_ynew   = 0.0;
		cellp_x1->v_ynew   = 0.0;
		cellp_x2->v_ynew   = 0.0;
		cellp_xcp3->v_ynew = cellp_xcp2->v_ynew;
		cellp_xcp4->v_ynew = cellp_xcp2->v_ynew;
		cellp_xcp5->v_ynew = cellp_xcp2->v_ynew;
	}
#endif
/*
#ifndef WALLX
...
*/
	...
	for(int i=0; i<(XCELL_NUM+6); i++){										//ï«ñ 
		...
		cellp_y3->v_xnew   = 0.0;
		cellp_ycp3->v_xnew = 0.0;
		cellp_ycp4->v_xnew = -cellp_ycp2->v_xnew;
		cellp_ycp5->v_xnew = -cellp_ycp1->v_xnew;
		cellp_y0->v_ynew   = -cellp_y6->v_ynew;
		cellp_y1->v_ynew   = -cellp_y5->v_ynew;
		cellp_y2->v_ynew   = -cellp_y4->v_ynew;
		cellp_y3->v_ynew   = 0.0;
		cellp_ycp3->v_ynew = 0.0;
		cellp_ycp4->v_ynew = -cellp_ycp2->v_ynew;
		cellp_ycp5->v_ynew = -cellp_ycp1->v_ynew;
	}
#endif
/*
#ifdef WALLY
...
void pressnewbnd(void){
	...
	for(int j=0; j<(YCELL_NUM+6); j++){
		...
		cellp_x0->pressnew   = pressstart;
		cellp_x1->pressnew   = pressstart;
		cellp_x2->pressnew   = pressstart;
		cellp_xcp3->pressnew = cellp_xcp2->pressnew;
		cellp_xcp4->pressnew = cellp_xcp2->pressnew;
		cellp_xcp5->pressnew = cellp_xcp2->pressnew;
	}
#endif
/*
#ifndef WALLX
...
*/
	...
	for(int i=0; i<(XCELL_NUM+6); i++){
		...
		cellp_ycp4->pressnew = cellp_ycp3->pressnew;
		cellp_ycp5->pressnew = cellp_ycp3->pressnew;
	}
#endif
/*
#ifdef WALLY
...
void rhonewbnd(void){
	...
	for(int j=0; j<(YCELL_NUM+6); j++){
		...
		cellp_x0->rhonew   = RHOmin;
		cellp_x1->rhonew   = RHOmin;
		cellp_x2->rhonew   = RHOmin;
		cellp_xcp3->rhonew = cellp_xcp2->rhonew;
		cellp_xcp4->rhonew = cellp_xcp2->rhonew;
		cellp_xcp5->rhonew = cellp_xcp2->rhonew;
	}
#endif
/*
#ifndef WALLX
...
*/
	...
	for(int i=0; i<(XCELL_NUM+6); i++){
		...
		if((cellp_ycp2->rhonew>RHOmin)&&(cellp_ycp2->rhonew<RHOmax)){
			cellp_ycp4->rhonew = 2.0*cellp_ycp3->rhonew-cellp_ycp2->rhonew;
			cellp_ycp5->rhonew = 2.0*cellp_ycp3->rhonew-cellp_ycp1->rhonew;
		}
		else {
			cellp_ycp4->rhonew = 2.0*cellp_ycp3->rhonew-cellp_ycp2->rhonew;
			cellp_ycp5->rhonew = 2.0*cellp_ycp3->rhonew-cellp_ycp1->rhonew;
...
void energynewbnd(void){
	...
	for(int j=0; j<(YCELL_NUM+6); j++){
		...
		cellp_x0->energynew   = energystart;
		cellp_x1->energynew   = energystart;
		cellp_x2->energynew   = energystart;
		cellp_xcp3->energynew = cellp_xcp2->energynew;
		cellp_xcp4->energynew = cellp_xcp2->energynew;
		cellp_xcp5->energynew = cellp_xcp2->energynew;
	}
#endif
/*
#ifndef WALLX
...
*/
	...
	for(int i=0; i<(XCELL_NUM+6); i++){
		...
		cellp_ycp4->energynew = 2.0*cellp_ycp3->energynew-cellp_ycp2->energynew;
		cellp_ycp5->energynew = cellp_ycp2->energynew;
	}
#endif
/*
#ifdef WALLY
...
void momentumbnd(void){
	...
	for (int j=0; j<(YCELL_NUM+6); j++){
		...
		cellp_x0->momentum_x   = RHOmin*Uinf*(1.0-((double)j - 28.0)/25.0*((double)j - 28.0)/25.0);
		cellp_x1->momentum_x   = RHOmin*Uinf*(1.0-((double)j - 28.0)/25.0*((double)j - 28.0)/25.0);
		cellp_x2->momentum_x   = RHOmin*Uinf*(1.0-((double)j - 28.0)/25.0*((double)j - 28.0)/25.0);
		cellp_xcp3->momentum_x = cellp_xcp2->momentum_x;
		cellp_xcp4->momentum_x = cellp_xcp2->momentum_x;
		cellp_xcp5->momentum_x = cellp_xcp2->momentum_x;
		cellp_x0->momentum_y   = 0.0;
		cellp_x1->momentum_y   = 0.0;
		cellp_x2->momentum_y   = 0.0;
		cellp_xcp3->momentum_y = cellp_xcp2->momentum_y;
		cellp_xcp4->momentum_y = cellp_xcp2->momentum_y;
		cellp_xcp5->momentum_y = cellp_xcp2->momentum_y;
	}
#endif
/*
#ifndef WALLX
...
/*
	...
	for(int i=0; i<(XCELL_NUM+6); i++){										//ï«ñ 
		...
		cellp_ycp3->momentum_x = 0.0;
		cellp_ycp4->momentum_x = -cellp_ycp2->momentum_x;
		cellp_ycp5->momentum_x = -cellp_ycp1->momentum_x;
		...
		cellp_ycp3->momentum_y = 0.0;
		cellp_ycp4->momentum_y = -cellp_ycp2->momentum_y;
		cellp_ycp5->momentum_y = -cellp_ycp1->momentum_y;
	}
#endif
/*
#ifdef WALLY
...
void vbnd(void){
	...
	for (int j=0; j<(YCELL_NUM+6); j++){
		...
		cellp_x0->v_x   = Uinf*(1.0-((double)j - 28.0)/25.0*((double)j - 28.0)/25.0);
		cellp_x1->v_x   = Uinf*(1.0-((double)j - 28.0)/25.0*((double)j - 28.0)/25.0);
		cellp_x2->v_x   = Uinf*(1.0-((double)j - 28.0)/25.0*((double)j - 28.0)/25.0);
...
/*
	...
	for(int i=0; i<(XCELL_NUM+6); i++){										//ï«ñ 
		...
		cellp_y3->v_x   = 0.0;
		cellp_ycp3->v_x = 0.0;
		cellp_ycp4->v_x = -cellp_ycp2->v_x;
		cellp_ycp5->v_x = -cellp_ycp1->v_x;
...
void pressbnd(void){
	...
	for(int j=0; j<(YCELL_NUM+6); j++){
		...
		cellp_x0->press   = pressstart;
		cellp_x1->press   = pressstart;
		cellp_x2->press   = pressstart;
...
/*
	...
	for(int i=0; i<(XCELL_NUM+6); i++){
		...
		cellp_y2->press   = cellp_y3->press;
		cellp_ycp4->press = cellp_ycp3->press;
		cellp_ycp5->press = cellp_ycp3->press;
	}
#endif
/*
#ifdef WALLY
...
void temperbnd(void){
	...
	for(int j=0; j<(YCELL_NUM+6); j++){
		...
		cellp_x2->tempernew   = Ts;
		cellp_xcp3->tempernew = cellp_xcp2->tempernew;
		cellp_xcp4->tempernew = cellp_xcp2->tempernew;
		cellp_xcp5->tempernew = cellp_xcp2->tempernew;
	}
#endif
/*
#ifndef WALLX
...
void rhobnd(void){
	...
	for(int j=0; j<(YCELL_NUM+6); j++){
		...
		cellp_xcp4->rho = cellp_xcp2->rho;
		cellp_xcp5->rho = cellp_xcp2->rho;
	}
#endif
/*
#ifndef WALLX
...
*/
	...
	for(int i=0; i<(XCELL_NUM+6); i++){
		...
		if((cellp_ycp2->rho>RHOmin)&&(cellp_ycp2->rho<RHOmax)){
			cellp_ycp4->rho = 2.0*cellp_ycp3->rho-cellp_ycp2->rho;
			cellp_ycp5->rho = 2.0*cellp_ycp3->rho-cellp_ycp1->rho;
		}
		else {
			cellp_ycp4->rho = 2.0*cellp_ycp3->rho-cellp_ycp2->rho;
			cellp_ycp5->rho = 2.0*cellp_ycp3->rho-cellp_ycp1->rho;
...
void energybnd(void){
	...
	for(int j=0; j<(YCELL_NUM+6); j++){
		...
		cellp_x0->energy   = energystart;
		cellp_x1->energy   = energystart;
		cellp_x2->energy   = energystart;
		cellp_xcp3->energy = cellp_xcp2->energy;
		cellp_xcp4->energy = cellp_xcp2->energy;
		cellp_xcp5->energy = cellp_xcp2->energy;
	}
#endif
/*
#ifndef WALLX
...
*/
	...
	for(int i=0; i<(XCELL_NUM+6); i++){
		...
		cellp_ycp3->energy = 2.0*cellp_ycp3->energy-cellp_ycp2->energy;
