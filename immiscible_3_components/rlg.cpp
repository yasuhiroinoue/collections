		...
		for( int col = 0; col < SPCS; col++){
			if( lst->pmap.count(col) != 0 ){
				...
				for( pos = lst->pmap.lower_bound(col); pos != lst->pmap.upper_bound(col); ++pos){
					...
					//êFèÍÇ©ÇÁéÛÇØÇÈóÕÇÕÅAêFÇÃÇ›Ç≈åàíËÇ≥ÇÍÇÈÇ◊Ç´ÅB
					//éøó ÇÕÅAäµê´â^ìÆÇÃïœâªÇÃÇµÇ…Ç≠Ç≥Ç»ÇÃÇ≈ÅAóÕÇéÛÇØÇΩå„ÇÃâ^ìÆèÛë‘ÇåàíËÇ∑ÇÈÇ±Ç∆ÇÕÇ†Ç¡ÇƒÇ‡
					//êFèÍÇ∆êFó±éqÇÃëäå›çÏópÇÃëÂÇ´Ç≥ÇåàÇﬂÇƒÇÕÇ»ÇÁÇ»Ç¢ÅB
...
void SystemMonitor(int step)
{
  ...
  fprintf(fptr, "%d %f %f %f %f\n",
	  /* ª˛¥÷     */ step,
	  /* ¡Ìº¡ŒÃ   */ total.mas,
	  /*  ø∂—≤π≈Ÿ */ total.ene/(2.0*(double)(ptcl_size)),
	  /*  ø∂—ŒÆ¬Æ */ total.mom.x, total.mom.y);
  fclose(fptr);
